    ...
    <>
      {showImport && (
        <CSVImport
          ...
          onImport={(rows) => {
            success(`${rows.length} contacts imported`, "Data added to your contacts list");
          }}
        />
      )}
            ...
            <div className="flex gap-3 items-center flex-wrap">
              <div className="relative flex-1 max-w-sm">
                ...
                <input
                  ...
                  className="w-full h-9 rounded-lg border border-input pl-9 pr-4 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-ring"
                />
              </div>
              ...
              <CardContent className="p-0">
                {filtered.length === 0 ? (
                  <EmptyState
                    ...
                    description={search ? `No results for "${search}"` : "Import contacts to get started."}
                    action={{ label: "Import CSV", onClick: () => setShowImport(true) }}
                  />
                ) : (
                      ...
                      <tbody className="divide-y divide-border">
                        {filtered.map((contact) => {
                          ...
                          return (
                            <tr
                              key={contact.id}
                              className={`hover:bg-muted/30 cursor-pointer ${selectedContact === contact.id ? "bg-primary/5" : ""}`}
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-input p-3 text-sm bg-background min-h-[80px] focus:outline-none focus:ring-2 focus:ring-ring"
                />
              </div>
            </div>
          </div>
