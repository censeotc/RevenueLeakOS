...
import { FileText, Clock, AlertTriangle, CheckCircle, Upload, Send } from "lucide-react";
  ...
  const handleEnroll = (id: string) => {
    setEnrolledIds((prev) => {
      ...
      if (next.has(id)) {
        ...
        next.add(id);
        const est = demoEstimates.find((e) => e.id === id);
        const contact = est ? getContactById(est.contactId) : null;
        success("Follow-up enrolled", `${contact?.firstName || "Contact"} added to follow-up sequence`);
      }
      return next;
    });
  };
    ...
    <>
      {showImport && (
        <CSVImport
          ...
          onImport={(rows) => {
            success(`${rows.length} estimates imported`, "Estimates added to your pipeline");
          }}
        />
      )}
            ...
            <CardContent className="p-0">
              {filtered.length === 0 ? (
                <EmptyState
                  ...
                  description={statusFilter !== "all" ? `No ${statusFilter} estimates.` : "Import estimates to get started."}
