...
import { Target, X, MessageSquare, User, Plus, Loader2, CheckCircle2, CalendarCheck } from "lucide-react";
import { sendSMS, buildMissedCallReply } from "@/services/twilioMockService";
...
export default function OpportunitiesPage() {
  ...
  const handleMarkContacted = async () => {
    ...
    try {
      await sendSMS({
        ...
        from: "+13135550100",
        body: buildMissedCallReply(selectedContact.firstName, "North Shore Heating & Plumbing", "(313) 555-0100"),
        opportunityId: selected.id,
      });
      setStatusOverrides((prev) => ({ ...prev, [selected.id]: "contacted" }));
      success("Marked as contacted", `SMS sent to ${selectedContact.firstName}`);
    } catch {
  ...
  const handleLogBooking = () => {
    if (!selected || !selectedContact) return;
    setBookingIds((prev) => new Set(prev).add(selected.id));
    setStatusOverrides((prev) => ({ ...prev, [selected.id]: "booked" }));
    success("Booking logged", `${selectedContact.firstName}'s appointment has been booked`);
  };
  ...
  return (
    <div className="flex h-full">
            ...
            <CardContent className="p-0">
              {filtered.length === 0 ? (
                <EmptyState
                  ...
                  description="No opportunities match the current filter."
                />
              ) : (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-border">
                      {filtered.map((opp) => {
                        ...
                        const effectiveStatus = statusOverrides[opp.id] ?? opp.status;
                        return (
                          <tr
                            key={opp.id}
                            className={`hover:bg-muted/30 cursor-pointer ${selectedOpp === opp.id ? "bg-primary/5" : ""}`}
                            onClick={() => setSelectedOpp(opp.id)}
                          >
                            <td className="px-4 py-3">
                              <div className="font-medium">{opp.title}</div>
                              <div className="text-xs text-muted-foreground truncate max-w-xs">{opp.description}</div>
                            </td>
                            <td className="px-4 py-3">
      ...
      {/* Detail Panel */}
      {selected && (
        <div className="w-full lg:w-[480px] border-l border-border bg-white overflow-y-auto flex flex-col">
          ...
          <div className="p-4 space-y-5 flex-1">
            ...
            <div>
              ...
              <select className="w-full rounded-lg border border-input px-3 py-2 text-sm bg-background">
                ...
                {demoUsers.map((u) => (
                  <option key={u.id} value={u.id} defaultValue={selected.assignedToId ?? undefined}>
                    {u.name}
                  </option>
                ))}
              </select>
            ...
            {bookingIds.has(selected.id) && (
              <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg border border-green-200">
                <CalendarCheck className="h-5 w-5 text-green-600" />
                <span className="text-sm font-medium text-green-800">Booking logged this session</span>
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg border border-input px-3 py-2 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-ring"
                />
          ...
          <div className="border-t border-border p-4 flex gap-2">
            <Button
              ...
              disabled={sendingMsg || !selectedContact || selectedStatus === "contacted" || selectedStatus === "booked" || selectedStatus === "won"}
            >
              {sendingMsg ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <CheckCircle2 className="h-4 w-4 mr-1" />}
              Mark Contacted
            </Button>
            <Button
              size="sm"
              ...
              disabled={bookingIds.has(selected.id) || !selectedContact}
            >
