...
export default function ReactivationPage() {
  ...
  const segments: Segment[] = [
    ...
    },
    {
      ...
      estimatedValue: maintenanceDue.length * 350,
    },
    {
      id: "membership",
      ...
      estimatedValue: membershipRenewal.length * 600,
    },
    {
      id: "replacement",
      ...
      estimatedValue: replacementCycle.length * 4500,
    },
  ];
  ...
  const handleLaunch = (seg: Segment, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    success(
      "Campaign launched",
      `Reactivation campaign started for ${seg.contacts.length} contacts in "${seg.name}"`
    );
  };
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {segments.map((seg) => (
            <Card
              key={seg.id}
              className={`cursor-pointer transition-all hover:shadow-md ${selectedSegment === seg.id ? "ring-2 ring-primary" : ""}`}
              onClick={() => setSelectedSegment(selectedSegment === seg.id ? null : seg.id)}
            >
              <CardContent className="pt-4">
                <div className="flex items-start justify-between">
                  <div className="rounded-lg p-2 bg-primary/10 text-primary">{seg.icon}</div>
                  <div className="text-right">
                    <span className="text-2xl font-bold">{seg.contacts.length}</span>
                    <div className="flex items-center gap-0.5 text-muted-foreground">
                      {selectedSegment === seg.id ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={(e) => handleLaunch(seg, e)}
                    disabled={launchedCampaigns.has(seg.id) || seg.contacts.length === 0}
                  >
        ...
        {activeSegment && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">{activeSegment.name} — {activeSegment.contacts.length} contacts</CardTitle>
            ...
            <CardContent className="p-0">
              {activeSegment.contacts.length === 0 ? (
                <EmptyState
                  ...
                  description="No contacts match these criteria right now."
                />
              ) : (
        ...
        {/* Active Reactivation Opportunities */}
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Active Reactivation Opportunities ({reactivationOpps.length})</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {reactivationOpps.length === 0 ? (
              <EmptyState
                ...
                description="Launch a campaign to create opportunities."
              />
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
